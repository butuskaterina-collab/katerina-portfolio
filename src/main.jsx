  ...
  return <>
    ...
    <main id="top">
      <section className="hero">
        ...
        <div className="hero-title" aria-label="Katerina Butusova">
          <motion.div initial={{ x: -40, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ duration: .8 }}>Katerina</motion.div>
          <motion.div initial={{ x: 50, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ duration: .9, delay: .08 }} className="indent">Butusova</motion.div>
        </div>
      ...
      <section className="section works" id="works">
        <div className="section-head"><div className="section-label">SELECTED WORKS / 01</div><a href="#contact">VIEW ALL ↗</a></div>
      ...
      <section className="section practice">
        ...
        <div className="practice-grid">
          {practice.map(([title, ...items]) => <div key={title}><b>{title}</b>{items.map(item => <React.Fragment key={item}><br />{item}</React.Fragment>)}</div>)}
        </div>
      </section>
